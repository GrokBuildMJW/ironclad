...
COMMAND_SPECS: Tuple[CommandSpec, ...] = (
    ...
    CommandSpec("approve", MUTATING,
                "Approve the active unit's design so implementation handovers are unblocked (S5, no blind coding).",
                flags=(FlagSpec("<slug>", "the unit to approve (default: active)", required=False),)),
    ...
    CommandSpec("fork", READ_ONLY, "Show the MPR architecture-decision proposal at a fork (read-only).",
                flags=(FlagSpec("[unit]",),)),
