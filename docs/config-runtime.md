...
> window, so a runtime `/config set` of those two keys is stored in the config tree but does **not** move the
> live global (it takes effect only with `context.token_budget=false`, or via `GX10_MAX_CTX_CHARS` /
> `GX10_TRIM_TARGET_CHARS` at boot). This is the MEM-9 budget model, not a failed apply.
...
<!-- Generated by scripts/ci/gen_config_runtime.py; do not edit this region. -->
...
<!-- Generated by scripts/ci/gen_config_runtime.py; parsed by check_config_switch_parity.py. -->
...
The detect-progress heartbeat is always on with a finite 900-second default. `heartbeat.stall_seconds` accepts
only a positive finite duration; zero, negative, non-finite, or malformed values are refused by the typed
schema and never disable the protection. A positive finite override remains live operational tuning.
...
> **Mandatory staging protections (#1466 F5a).** Task validation, required handover verification, the
...
> **Finite failure and heartbeat protections (#1466 F5b).** Failure classification, per-task attempt
...
> `heartbeat.stall_seconds` remain bounded positive tuning; the heartbeat deliberately does not infer a stall
> for a task that has never produced a coder-log or feedback signal.
