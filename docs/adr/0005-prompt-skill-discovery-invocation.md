...
- **Status:** Accepted (implemented) — discovery, invocation, catalogue/autocomplete, and seed all delivered. For what ships see [`status.md`](../status.md). Closes the slash-resolution gap left open by [ADR-0003](0003-prompt-library.md) D5.
- **Date:** 2026-06-22
...
After [ADR-0003](0003-prompt-library.md), prompts ship as always-on core built-ins but were only reachable through the **model-elected `use_prompt` tool** — there was **no way to list the loaded library and no deterministic per-item invocation**, and slash autocomplete was a static, hand-maintained list. ADR-0003 **D5** specified a router slash-resolution surface, but the initial delivery shipped only the tool. This work makes the delivered library **usable, discoverable, and seeded**, reading the **one** registry the loaders already populate — no second mechanism.
...
**D5 — Seed selection criteria.** The curated starter set is small (target 5–8), general-purpose, declarative (`kind: prompt`, one MD file + a `locales/de.json` overlay), **multilingual (EN+DE)**, and **must pass `ack.gate`** (required variables used in the template; each declared language assembles cleanly). No blind import of foreign-ecosystem skills. This work brought the set to **7** (`code-review`, `commit-message`, `bug-report`, `explain-code`, `pr-description`, `refactor-plan`, `test-plan`). "New prompt = drop an MD file" — no engine change.
...
- The intelligent **curation/research/auto-maintain** flow (and "save assembled prompt as an item") is explicitly out of scope — a separate follow-up.
