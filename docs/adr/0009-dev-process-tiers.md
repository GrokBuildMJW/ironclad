...
**Status:** accepted (epic #532, S1/#537). Builds on ADR-0004 (extension SDK) and the dev-process guard. **Amended by [ADR-0011](0011-dev-process-rework-project-isolation.md)** (epic #601): DEV-2 and tier 3 merge to a single native tier, and locality (in-engine vs github) becomes an orthogonal `exec_mode` axis rather than a tier. **The resolver/presets described below are now PRIVATE** (`scripts/devprocess`, not the wheel); the public dev-process surface is the three DEV-1 prompts only — the `ack.devprocess` / "in the wheel" wording below is historical.

## Context

Epic #532 ports the end-to-end development process into Ironclad as **three switchable tiers**
(operator decisions OD1–OD11):
...
A tier must be *selectable* and the selection must actually gate which capabilities run (OD4/OD10).
...
| flag | values | meaning |
|---|---|---|
| `guards` | `off` \| `native` | the native composed GATE + coupling guards (DEV2-R1/R2) |
| `push` | `off` \| `switchable` \| `on` | the GitHub-adapter seam (`switchable` = provided-but-opt-in, DEV2-R6) |
...
**DEV-1 is stateless (S4 / DEV1-R5, OD5).** DEV-1 has **no work-item store / backlog** — it is a
...
**Runtime-guard binding (S9 / DEV2-R3).** `runtime_guards(tier)` ties the #440 engine runtime guards
...
- The three tiers are presets over composable flags (OD4(c)/OD10(c)) — the default UX is the named
