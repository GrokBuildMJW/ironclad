...
- **Status:** Accepted (design) — implementation tracked separately. Records the design; for what ships see [`status.md`](../status.md). The D5 slash-command surface (discovery + per-item `/<prompt-name>` invocation) is delivered by [ADR-0005](0005-prompt-skill-discovery-invocation.md).
- **Date:** 2026-06-21
- **Context sources:** an internal research catalogue (prompt/skill patterns), `skills/mpr/` (the real skill format + `i18n` usage), and the **core base** from [ADR-0002](0002-core-always-on-skills.md) (always-on built-in loader, shared `ack.i18n`, `ack.playbook`, `ack.catalogue`, `ack.gate`).

## Context

Make prompting easy: a curated, multilingual **prompt library** + a generator. A user invokes a prompt as a slash-command ("prompt as a function"), is **guided (elicitation)** through intent/variables, and gets a finished, high-quality prompt in the target language. With the substrate now **core, always-on**, built-ins load from `skills/` (no `GX10_PLUGINS_DIR`), content i18n is `ack.i18n`, and `ack.catalogue`/`ack.gate`/`ack.playbook` exist. So the prompt library is built **on the core engine, as a core built-in — no plugin, no parallel infrastructure.**

There is **no existing prompt library to migrate** (verified by grep — only the orchestrator system prompt + MPR's synthesis-internal prompt construction exist); the i18n migration was already done as part of the always-on core work. So this work is additive.

## Decisions

**D1 — A prompt is a declarative MD item, `kind: prompt`, a core built-in.** It lives under `skills/` (always-on via the built-in loader), reuses the `ack.playbook` parser (frontmatter + body + lazy references), is indexed by `ack.catalogue` (semver + provenance), and is validated by `ack.gate`. A **Python prompt** (`skills/mpr`-style) remains available for coding cases (dual format). **No new registry/loader; no plugin.**
...
- The user-defined / 3rd-party prompt libraries ride the existing plugin surface (`GX10_PLUGINS_DIR`) later (a soft dependency on the per-principal / multi-tenant work).
...
- **A separate prompt registry/loader/i18n** — rejected: parallel infrastructure, against the core-reuse mandate.
- **Plugin-loaded prompts** — rejected: prompts are first-class built-ins now, always-on; 3rd-party prompts still ride the plugin surface.
